...
Create a local `.env` file and set `COOKIE_VALIDATION_KEY` to a long random secret:

```dotenv
COOKIE_VALIDATION_KEY=<secret random string goes here>
```

The `.env` file is loaded by the application and Docker Compose and is ignored by Git. A real environment
variable with the same name takes precedence over the value from the file.
